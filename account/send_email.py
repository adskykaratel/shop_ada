...
def send_confirmation_email(email , code):
    ...
    html_message = render_to_string('account2/activate.html',context)
...
def send_activation_sms(phone_number,activation_code):
    message = f'Ваш код для активации:{activation_code}'
...
def send_confirmation_password(email,code):
    ...
    context = {'activation_url': activation_url}
    subject = 'Подтвердите изменения пороля'
    html_message = render_to_string('account2/new_password.html',context )
    plain_message = strip_tags(html_message)
